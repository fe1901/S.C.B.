...
void loop() {
  ...
  if (selecao == 'k') {
    ...
    // Pequeno delay para leitura humana, mas cuidado com timeout do serial
    delay(1000); 

    // Espera o Python enviar o segundo byte (o ID)
    while (Serial.available() == 0);

    // LÊ O ID DIRETAMENTE (Sem converter de ASCII)
    // O Python enviou bytes([5]), então aqui lemos o valor 5.
    uint8_t idParaDeletar = Serial.read();

    // Verificação de segurança (IDs válidos geralmente são > 0)
